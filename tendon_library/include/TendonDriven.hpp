...
public:
	...
	TendonDriven(double E, double G, double rad, double mass, double L, double alpha, double dt, double tendonOffset, double tendonCompliance, double basePlateToMotor, double dampBendTwist, double dampShearExt, double dragCoeff);
	...
	template <mathOp::cosseratEquations solType, mathOp::integrationMethod intMethod>
	bool Levenberg_Marquardt_II(blaze::StaticVector<double, 6UL + numTendons> &initGuess);
	...
	// method for setting the actuation input for the catheter
	void setActuationInput(const blaze::StaticVector<double, numTendons> &q);
	...
	std::tuple<blaze::StaticVector<double, 3UL>, blaze::StaticVector<double, 4UL>> getTipPose();
	...
	std::tuple<blaze::StaticVector<double, 3UL>, blaze::StaticVector<double, 3UL>, blaze::StaticVector<double, numTendons>, blaze::StaticVector<double, numTendons>> getTrainingData();
...
private:
	...
	blaze::StaticVector<double, numTendons> m_q;						// joint actuation value ==> tendon displacement [m]
	blaze::StaticVector<double, numTendons> m_tau;						// jcorresponding tendon tension [N]
	blaze::StaticVector<double, numTendons> m_tendonSlack;				// tensile actuation force on tendons
...
// overloaded constructor
template <std::size_t N, std::size_t numTendons>
TendonDriven<N, numTendons>::TendonDriven(double E, double G, double rad, double mass, double L, double alpha, double dt, double tendonOffset, double tendonCompliance, double basePlateToMotor, double dampBendTwist, double dampShearExt, double dragCoeff) : m_E(E), m_G(G), m_rad(rad), m_mass(mass), m_L(L), m_alpha(alpha), m_dt(dt), m_tendonOffset(tendonOffset), m_basePlateToMotor(basePlateToMotor), m_tendonCompliance(tendonCompliance)
{
...
// copy constructor
template <std::size_t N, std::size_t numTendons>
TendonDriven<N, numTendons>::TendonDriven(const TendonDriven &rhs) : m_E(rhs.m_E), m_G(rhs.m_G), m_rad(rhs.m_rad), m_mass(rhs.m_mass), m_L(rhs.m_L), m_alpha(rhs.m_alpha), m_dt(rhs.m_dt), m_tendonOffset(rhs.m_tendonOffset), m_tendonAngleOffset(rhs.m_tendonAngleOffset), m_A(rhs.m_A), m_rho(rhs.m_rho), m_Kse(rhs.m_Kse), m_Kbt(rhs.m_Kbt), m_J(rhs.m_J), m_BVPAccuracy(rhs.m_BVPAccuracy), m_tendonSlack(rhs.m_tendonSlack), m_Bse(rhs.m_Bse), m_Bbt(rhs.m_Bbt), m_C(rhs.m_C), m_r(rhs.m_r), m_g(rhs.m_g), m_f(rhs.m_g), m_basePlateToMotor(rhs.m_basePlateToMotor), m_tendonCompliance(rhs.m_tendonCompliance), m_tau(rhs.m_tau)
{
...
blaze::StaticVector<double, 6UL + numTendons> TendonDriven<N, numTendons>::residueFunction(const blaze::StaticVector<double, 6UL + numTendons> &initGuess)
{
	...
	blaze::StaticVector<double, 3UL> e3 = {0.00, 0.00, 1.00};
	blaze::StaticVector<double, 3UL> v0 = e3 + blaze::inv(this->m_Kse) * blaze::subvector<0UL, 3UL>(initGuess); // v = v* + K^(-1)*R^T*n
	blaze::StaticVector<double, 3UL> u0 = blaze::inv(this->m_Kbt) * blaze::subvector<3UL, 3UL>(initGuess);		// u = u* + K^(-1)*R^T*m
	double c0 = this->m_time->getC0();
	// initGuess := [internal force n(0), internal moment m(0), tendon_lengths]
	this->m_tau = blaze::map(blaze::subvector<6UL, numTendons>(initGuess), [](double d)
							 { return std::max(d, 0.00); }); // tendon-displacement control => tau >= 0.0
	this->m_tendonSlack = blaze::map(blaze::subvector<6UL, numTendons>(initGuess), [](double d)
									 { return std::fabs(std::min(d, 0.00)); }); // tension and slack are mutually exclusive => slack < 0
	...
	blaze::subvector<19UL, numTendons>(dyds) = blaze::uniform(numTendons, this->m_basePlateToMotor); // initial tendon length
	...
	case mathOp::cosseratEquations::DYNAMIC_SOLUTION:
		...
		{
		case mathOp::integrationMethod::RK4:
			this->m_PDE->rungeKutta_PDE(this->m_y, dyds, this->m_zh, this->m_z, this->m_L, c0, this->m_Kse, this->m_Kbt, this->m_Bse, this->m_Bbt, this->m_C, this->m_J, this->m_r, this->m_tau, this->m_A, this->m_rho, this->m_g);
			break;
		case mathOp::integrationMethod::EULER:
			this->m_PDE->euler_PDE(this->m_y, dyds, this->m_zh, this->m_z, this->m_L, c0, this->m_Kse, this->m_Kbt, this->m_Bse, this->m_Bbt, this->m_C, this->m_J, this->m_r, this->m_tau, this->m_A, this->m_rho, this->m_g);
			break;
		}
		break;
	default:
	...
	blaze::StaticVector<double, 3UL> vL, uL, vL_t, uL_t, qL, wL, qL_t, wL_t;
	...
	case mathOp::cosseratEquations::STATIC_SOLUTION:
		...
		break;
	case mathOp::cosseratEquations::DYNAMIC_SOLUTION:
		vL_t = (c0 * vL) + blaze::subvector<0UL, 3UL>(blaze::column<N - 1UL>(this->m_zh));
		uL_t = (c0 * uL) + blaze::subvector<3UL, 3UL>(blaze::column<N - 1UL>(this->m_zh));
		// linear velocity at the distal-end
		qL = blaze::subvector<13UL, 3UL>(blaze::column<N - 1UL>(this->m_y));
		// angular velocity at the distal-end
		wL = blaze::subvector<16UL, 3UL>(blaze::column<N - 1UL>(this->m_y));
		// linear acceleration at the distal-end
		qL_t = (c0 * qL) + blaze::subvector<6UL, 3UL>(blaze::column<N - 1UL>(this->m_zh));
		wL_t = (c0 * wL) + blaze::subvector<9UL, 3UL>(blaze::column<N - 1UL>(this->m_zh));
		break;
	}
	// nb, mb
	blaze::StaticVector<double, 3UL> nb = this->m_Kse * (vL - e3) + this->m_Bse * vL_t; // internal force at distal end
	blaze::StaticVector<double, 3UL> mb = this->m_Kbt * uL + this->m_Bbt * uL_t;		// internal moment at distal end
	...
	/*
	======================= Accounting for Distal-End Rigid Body Dynamics =======================
	*/
	// Distal-end orientarion
	blaze::StaticVector<double, 4UL> hL = blaze::subvector<3UL, 4UL>(blaze::column<N - 1>(this->m_y));
	blaze::StaticMatrix<double, 3UL, 3UL> R_e;
	// Acquiring the distal-end orientation in SO(3)
	mathOp::getSO3(hL, R_e);
	// end-effector mass
	double m_e = 1.04E-3;
	// Inertia tensor of the end-effector
	blaze::StaticMatrix<double, 3UL, 3UL> J_e = {
		{42.43, 0.00, -1.60},
		{0.00, 42.80, 00.00},
		{-1.60, 00.00, 0.65}};
	// transforming the inertia tensor to SI units
	J_e *= 1.00E-9;
	// account for the external and distal-end inertial forces
	forceError += this->m_f + m_e * this->m_g - m_e * qL_t;
	// account for the external and distal-end inertial moments
	momentError -= R_e * J_e * blaze::trans(R_e) * wL_t + mathOp::hatPreMultiply(wL, R_e) * J_e * blaze::trans(R_e) * wL;
	// compute the violation in tendon Lengths (as the catheter is tendon displacement controlled)
	blaze::StaticVector<double, numTendons> length, lengthStar, lengthError, stretch;
	// integrated lengths of the tendons
	length = blaze::subvector<19UL, numTendons>(blaze::column<N - 1>(this->m_y));
	// The reference tendon length is l∗(t) = L + basePlateToMotor + q(t)
	lengthStar = blaze::uniform(numTendons, this->m_L + this->m_basePlateToMotor) + this->m_q;
...
{
	const size_t sz = 6UL + numTendons;
...
bool TendonDriven<N, numTendons>::Levenberg_Marquardt(blaze::StaticVector<double, 6UL + numTendons> &initGuess)
{
	...
	size_t k = 0UL;
	const size_t k_max = 300UL, sz = 6UL + numTendons;
...
template <std::size_t N, std::size_t numTendons>
template <mathOp::cosseratEquations solType, mathOp::integrationMethod intMethod>
bool TendonDriven<N, numTendons>::Levenberg_Marquardt_II(blaze::StaticVector<double, 6UL + numTendons> &initGuess)
{
	initGuess = blaze::map(initGuess, [](double d)
						   { return (!blaze::isfinite(d)) ? 0.00 : blaze::sign(d) * std::min(blaze::abs(d), 70.00); });
...
bool TendonDriven<N, numTendons>::Modified_Newton_Raphson(blaze::StaticVector<double, 6UL + numTendons> &initGuess)
{
	...
	*/
	...
	auto readjustInitialGuesses = [](blaze::StaticVector<double, 6UL + numTendons> &initial_guesses)
	{
...
// method for setting the actuation input for the catheter
template <std::size_t N, std::size_t numTendons>
void TendonDriven<N, numTendons>::setActuationInput(const blaze::StaticVector<double, numTendons> &q)
{
	this->m_q = q;
}
...
// method for returning the distal tip position & orientation of the tendon-driven robot
template <std::size_t N, std::size_t numTendons>
std::tuple<blaze::StaticVector<double, 3UL>, blaze::StaticVector<double, 4UL>> TendonDriven<N, numTendons>::getTipPose()
{
	const blaze::StaticVector<double, 3UL> position = blaze::subvector<0UL, 3UL>(blaze::column<N - 1>(this->m_y));
	const blaze::StaticVector<double, 4UL> orientation = blaze::subvector<3UL, 4UL>(blaze::column<N - 1>(this->m_y));
...
template <std::size_t N, std::size_t numTendons>
std::tuple<blaze::StaticVector<double, 3UL>, blaze::StaticVector<double, 3UL>, blaze::StaticVector<double, numTendons>, blaze::StaticVector<double, numTendons>> TendonDriven<N, numTendons>::getTrainingData()
{
	const blaze::StaticVector<double, 4UL> h = blaze::subvector<3UL, 4UL>(blaze::column<N - 1>(this->m_y));
	blaze::StaticMatrix<double, 3UL, 3UL> R;
	mathOp::getSO3(h, R);

	const blaze::StaticVector<double, 3UL> position = blaze::subvector<0UL, 3UL>(blaze::column<N - 1>(this->m_y));
	const blaze::StaticVector<double, 3UL> velocity = R * blaze::subvector<13UL, 3UL>(blaze::column<N - 1>(this->m_y));
	// const blaze::StaticVector<double, numTendons> tension = blaze::column<N - 1>(this->m_y);

	return std::make_tuple(position, velocity, this->m_tau, this->m_q);
}