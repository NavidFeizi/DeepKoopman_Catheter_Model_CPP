...
public:
	...
	std::tuple<blaze::StaticVector<double, 19UL + numTendons>, blaze::StaticVector<double, numTendons>, blaze::StaticVector<double, numTendons>> getTrainingData();
...
template <std::size_t N, std::size_t numTendons>
std::tuple<blaze::StaticVector<double, 19UL + numTendons>, blaze::StaticVector<double, numTendons>, blaze::StaticVector<double, numTendons>> TendonDriven<N, numTendons>::getTrainingData()
{
    blaze::StaticVector<double, 19UL + numTendons> states;
	blaze::StaticVector<double, numTendons> tension;
	blaze::StaticVector<double, numTendons> displacement;

    // double c0 = this->m_time->getC0();

    // acquiring the values of y
    subvector(states, 0UL, 19UL + numTendons) = blaze::column<N - 1>(this->m_y);
	subvector(tension, 0UL, numTendons) = this->m_tau;
	subvector(displacement, 0UL, numTendons) = this->m_q;

    return std::make_tuple(states, tension, displacement);
}